# george:
# not really a fan of how this library is written and would like to replace it someday.
# had to write my own addPitchBendEvent method
# and it's hosted on google code svn so had to copy paste :-\
...
class MIDITrack:
    ...
    # ugh
    class PitchBendEvent(GenericEvent):
        
        def __init__(self, channel, time, bend):
            GenericEvent.__init__(self, time,)
            self.type = 'pitchBend'
            self.pitchLSB = bend & 0x7F
            self.pitchMSB = (bend >> 7) & 0x7F
            self.channel = channel
    ...
    def addPitchBendEvent(self,channel,time, paramerter1):
        self.eventList.append(MIDITrack.PitchBendEvent(channel,time, paramerter1))
    ...
    def processEventList(self):
        ...
        for thing in self.eventList:
            ...
            elif thing.type == 'pitchBend':
                event = MIDIEvent()
                event.type = "PitchBend"
                event.time = thing.time * TICKSPERBEAT
                event.channel = thing.channel
                event.pitchLSB = thing.pitchLSB
                event.pitchMSB = thing.pitchMSB
                event.ord = 1
                self.MIDIEventList.append(event)
        ...
        for event in self.MIDIEventList:
            if event.type == "NoteOn":
                ...
                for timeByte in varTime:
                    self.MIDIdata = self.MIDIdata + struct.pack('>B',timeByte)
                self.MIDIdata = self.MIDIdata + struct.pack('>B',code)
                self.MIDIdata = self.MIDIdata + struct.pack('>B',event.pitchLSB)
                self.MIDIdata = self.MIDIdata + struct.pack('>B',event.pitchMSB)
...
class MIDIFile:
    ...
    def addPitchBendEvent(self, track, channel, time, paramerter1):

        self.tracks[track].addPitchBendEvent(channel, time, paramerter1)
